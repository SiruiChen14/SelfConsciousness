...
save_path = os.path.join(base_path, 'exp', exp_name + '_head')
models_save_dir = os.path.join(save_path, "models")

os.makedirs(save_path, exist_ok=True)
os.makedirs(models_save_dir, exist_ok=True)

features = np.load(os.path.join(base_path, 'features', exp_name + '_head_wise.npy'))
...
for i in range(num_layers):
    model_save_path = os.path.join(models_save_dir, f'model_layer_{i}.pth')
    accuracy = train_and_evaluate(train_loader, test_loader, i, model_save_path)
    print(f"Layer {i} accuracy: {accuracy}")
    accuracies.append(accuracy)

with open(os.path.join(save_path, exp_name + '_head_wise_accuracies.json'), 'w') as json_file:
